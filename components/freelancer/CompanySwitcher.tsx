...
import { setActiveCompany } from "@/app/freelancer/(protected)/mere/actions";
import type { ActiveCompany } from "@/lib/freelancer";
      ...
      >
        <div className="w-9 h-9 rounded-[9px] bg-pepo-su border border-pepo-bd flex items-center justify-center overflow-hidden flex-shrink-0">
          {active?.logo_url ? (
            ...
            <img src={active.logo_url} alt="" className="w-full h-full object-contain" />
          ) : (
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[13.5px] font-medium text-pepo-t1 truncate">{active?.name}</div>
        </div>
        {isPending ? (
        ...
        <div className="border-t border-pepo-bd divide-y divide-pepo-bd">
          {companies.map((c) => (
            ...
            >
              ...
              <span className="flex-1 text-[13.5px] text-pepo-t1 truncate">{c.name}</span>
              {c.id === selectedId && <Icon name="check" size={16} className="text-pepo-p flex-shrink-0" />}
            </button>
          ))}
        </div>
      )}
